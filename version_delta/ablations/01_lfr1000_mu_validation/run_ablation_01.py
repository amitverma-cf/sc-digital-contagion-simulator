...
Generates multiple LFR candidates, accepts one within tolerance, then runs Delta baseline vs intervention.
Outputs are written only inside this ablation folder.
"""
...
def build_lfr_graph(seed):
    G = nx.LFR_benchmark_graph(
        n=LFR_CFG["n"],
        tau1=LFR_CFG["tau1"],
        tau2=LFR_CFG["tau2"],
        mu=LFR_CFG["mu"],
        average_degree=LFR_CFG["average_degree"],
        min_community=LFR_CFG["min_community"],
        max_community=LFR_CFG["max_community"],
        max_iters=5000,
        seed=seed,
    )
...
def compute_topology_metrics(G, actual_mu, community_id_by_node):
    ...
    metrics = {
        ...
        "target_mixing_parameter": float(LFR_CFG["mu"]),
        "mixing_parameter_distance": float(abs(actual_mu - LFR_CFG["mu"])),
        "mixing_tolerance": float(MU_TOLERANCE),
        "mixing_tolerance_met": bool(abs(actual_mu - LFR_CFG["mu"]) <= MU_TOLERANCE),
...
def plot_trajectory(summary_df, output_path):
    ...
    plt.title("Ablation 01: Stress Trajectory (Mean ± SD)")
    plt.xlabel("Day")
...
def plot_burnout(summary_df, output_path):
    ...
    plt.axvline(x=SIM_PARAMS["intervention_day"], color="black", linestyle="--", alpha=0.3)
    plt.title("Ablation 01: Burnout Count (Mean ± SD)")
    plt.xlabel("Day")
...
def main():
    ...
    for i in range(MAX_ATTEMPTS):
        seed = LFR_CFG["seed_start"] + i
        try:
            G, actual_mu, community_id_by_node = build_lfr_graph(seed)
        ...
        metrics = compute_topology_metrics(G, actual_mu, community_id_by_node)
        candidate_rows.append({
            "seed": seed,
        ...
        if metrics["mixing_tolerance_met"]:
            accepted = (G, metrics, community_id_by_node, seed)
            break

    candidates_df = pd.DataFrame(candidate_rows)
    candidates_df.to_csv(ABLATION_DIR / "ablation01_candidates.csv", index=False)

    if accepted is None:
        return

    G, topo_metrics, community_id_by_node, seed_used = accepted
    ...
    nx.write_graphml(G, ABLATION_DIR / "ablation01_network.graphml")
    with open(ABLATION_DIR / "ablation01_topology_metrics.json", "w", encoding="utf-8") as f:
    ...
    with open(ABLATION_DIR / "ablation01_targets.json", "w", encoding="utf-8") as f:
    ...
    records_df.to_csv(ABLATION_DIR / "ablation01_simulation_records.csv", index=False)
    metrics_df.to_csv(ABLATION_DIR / "ablation01_run_metrics.csv", index=False)
    ...
    with open(ABLATION_DIR / "ablation01_aggregated_metrics.json", "w", encoding="utf-8") as f:
        json.dump(aggregated_metrics, f, indent=2)

    daily_summary = build_daily_summary(records_df)
    daily_summary.to_csv(ABLATION_DIR / "ablation01_daily_summary.csv", index=False)

    plot_trajectory(daily_summary, ABLATION_DIR / "ablation01_trajectory.png")
    plot_burnout(daily_summary, ABLATION_DIR / "ablation01_burnout.png")
